...
const LicenseEditModel = (rowData) => {
  ...
  const onSubmit = async (data) => {
    if (data.name) {
      const res = await axios.patch("/admin/license", { _id: rowData.value, ...data });
      if (res.status === 200) {
        const allLicense = await axios.get("/admin/license");
        dispatch(addLicense(allLicense.data));
        onHide();
      }
    }
  };
  ...
  return (
    <>
      <DiaLogComponent
        ...
        style={{ height: "100%" }}
      >
        <SingleFieldUpdateForm rowData={rowData} control={control} name="name" label="License Name"/>
      </DiaLogComponent>
...
export default LicenseEditModel;
